...
// Map of browser ws -> connId -> target ws
...
wss.on("connection", (clientWs, req) => {
  ...
  clientWs.on("message", async (data) => {
    ...
    if (type === "open") {
      ...
      serverWs.on("message", (msgData) => {
        if (msgData instanceof Buffer) {
          clientWs.send(JSON.stringify({ type: "message", connId, isBinary: true, payload: msgData.toString("base64") }));
        } else {
      ...
      if (isBinary) serverWs.send(Buffer.from(payload, "base64"));
...
server.listen(PORT, '0.0.0.0', () => console.log(`[relay DEBUG] Listening on port ${PORT}`));
